...
def init_db():
    ...
    c.execute('''CREATE TABLE IF NOT EXISTS users
                 (username TEXT PRIMARY KEY, password TEXT, role TEXT,
                  ...
                  last_active TEXT, level INTEGER DEFAULT 1,
                  badges TEXT DEFAULT '[]', total_messages INTEGER DEFAULT 0,
                  joined TEXT)''')
...
def add_user(username, password, role="student"):
    ...
    try:
        c.execute('INSERT INTO users(username, password, role, xp, streak, last_active, level, badges, total_messages, joined) VALUES (?,?,?,?,?,?,?,?,?,?)',
...
def view_all_users():
    conn = sqlite3.connect('zukko_school.db')
    df = pd.read_sql_query("SELECT username, role, xp, level, streak, total_messages, joined FROM users", conn)
    conn.close()
    return df
...
def add_xp(username, amount):
    conn = sqlite3.connect('zukko_school.db')
    c = conn.cursor()
    c.execute('UPDATE users SET xp = xp + ?, total_messages = total_messages + 1 WHERE username = ?', (amount, username))
...
def get_user_stats(username):
    conn = sqlite3.connect('zukko_school.db')
    c = conn.cursor()
    c.execute('SELECT xp, streak, level, badges, total_messages, joined FROM users WHERE username = ?', (username,))
    row = c.fetchone()
    conn.close()
    if row:
        return {"xp": row[0], "streak": row[1], "level": row[2],
                "badges": json.loads(row[3]) if row[3] else [],
                "total_messages": row[4], "joined": row[5]}
    return {"xp": 0, "streak": 0, "level": 1, "badges": [], "total_messages": 0, "joined": ""}
...
def update_streak(username):
    ...
    if row and row[0]:
        try:
            ...
            diff = (now - last).days
            if diff == 1:
                c.execute('UPDATE users SET streak = streak + 1, last_active = ? WHERE username = ?',
                          (now.strftime("%Y-%m-%d %H:%M:%S"), username))
            elif diff > 1:
                c.execute('UPDATE users SET streak = 1, last_active = ? WHERE username = ?',
                          (now.strftime("%Y-%m-%d %H:%M:%S"), username))
            else:
...
def check_achievements(username):
    ...
    earned = []
    if stats["total_messages"] >= 1 and "🌟 Birinchi Qadam" not in stats["badges"]:
        add_badge(username, "🌟 Birinchi Qadam")
        earned.append("🌟 Birinchi Qadam")
    if stats["total_messages"] >= 10 and "💬 Suhbatdosh" not in stats["badges"]:
        add_badge(username, "💬 Suhbatdosh")
        earned.append("💬 Suhbatdosh")
    if stats["total_messages"] >= 50 and "🔥 Faol O'quvchi" not in stats["badges"]:
        add_badge(username, "🔥 Faol O'quvchi")
        earned.append("🔥 Faol O'quvchi")
    if stats["total_messages"] >= 100 and "🏆 Zukko Master" not in stats["badges"]:
        add_badge(username, "🏆 Zukko Master")
        earned.append("🏆 Zukko Master")
    if stats["streak"] >= 3 and "📅 3 Kunlik Streak" not in stats["badges"]:
        add_badge(username, "📅 3 Kunlik Streak")
        earned.append("📅 3 Kunlik Streak")
    if stats["streak"] >= 7 and "🔥 Haftalik Streak" not in stats["badges"]:
        add_badge(username, "🔥 Haftalik Streak")
        earned.append("🔥 Haftalik Streak")
    if stats["level"] >= 5 and "⭐ 5-Daraja" not in stats["badges"]:
        add_badge(username, "⭐ 5-Daraja")
        earned.append("⭐ 5-Daraja")
    if stats["level"] >= 10 and "👑 10-Daraja" not in stats["badges"]:
        add_badge(username, "👑 10-Daraja")
        earned.append("👑 10-Daraja")
    return earned
...
def save_note(username, title, content, subject):
    ...
    c = conn.cursor()
    now = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    c.execute('INSERT INTO notes(username, title, content, subject, time) VALUES (?,?,?,?,?)',
...
def get_notes(username):
    conn = sqlite3.connect('zukko_school.db')
    df = pd.read_sql_query("SELECT id, title, subject, time FROM notes WHERE username = ? ORDER BY time DESC", conn, params=(username,))
    conn.close()
    return df
...
def get_note_content(note_id):
    conn = sqlite3.connect('zukko_school.db')
    c = conn.cursor()
    c.execute('SELECT title, content, subject, time FROM notes WHERE id = ?', (note_id,))
    row = c.fetchone()
    conn.close()
    return row
...
def save_quiz_score(username, subject, score, total):
    ...
    c = conn.cursor()
    now = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    c.execute('INSERT INTO quiz_scores(username, subject, score, total, time) VALUES (?,?,?,?,?)',
...
def get_quiz_history(username):
    conn = sqlite3.connect('zukko_school.db')
    df = pd.read_sql_query("SELECT subject, score, total, time FROM quiz_scores WHERE username = ? ORDER BY time DESC LIMIT 20", conn, params=(username,))
    conn.close()
    return df

def get_leaderboard():
    conn = sqlite3.connect('zukko_school.db')
    df = pd.read_sql_query("SELECT username, xp, level, streak FROM users WHERE role != 'admin' ORDER BY xp DESC LIMIT 10", conn)
    conn.close()
    return df
...
def show_dashboard(username):
    ...
    <div style="margin: 16px 0;">
        ...
        <div class="progress-container">
            <div class="progress-bar" style="width: {xp_in_level}%;">{xp_in_level}%</div>
        </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    with col_a:
        ...
        if stats["badges"]:
            badges_html = "".join([f'<span class="badge-item">{b}</span>' for b in stats["badges"]])
            st.markdown(f'<div class="badge-box">{badges_html}</div>', unsafe_allow_html=True)
        else:
            st.markdown("""
            <div class="badge-box">
...
def show_leaderboard():
    st.markdown('<h2 style="text-align:center;">🏆 Top O\'quvchilar Reytingi</h2>', unsafe_allow_html=True)
    st.markdown('<div class="fancy-divider"></div>', unsafe_allow_html=True)
    ...
    for i, row in df.iterrows():
        ...
        <div class="leader-row">
            ...
            <div class="leader-xp">⚡{row['xp']} XP · Lvl {row['level']} · 🔥{row['streak']}</div>
        </div>""", unsafe_allow_html=True)
...
def show_notes(username):
    ...
    with tab_add:
        with st.form("note_form"):
            title = st.text_input("Sarlavha", placeholder="Masalan: Python asoslari")
            subject = st.selectbox("Fan", ["Umumiy", "Ingliz tili", "IT", "Ona tili", "Matematika", "Fizika", "Boshqa"])
            content = st.text_area("Matn", height=200, placeholder="Eslatma matnini yozing...")
    ...
    with tab_view:
        ...
        if notes_df.empty:
            ...
            for _, row in notes_df.iterrows():
                with st.expander(f"📌 {row['title']} — [{row['subject']}] — {row['time'][:10]}"):
                    note_data = get_note_content(row['id'])
                    if note_data:
                        st.markdown(note_data[1])
                        col_d1, col_d2 = st.columns([4, 1])
                        with col_d2:
                            if st.button("🗑️ O'chirish", key=f"del_{row['id']}"):
                                delete_note(row['id'])
                                st.rerun()
...
def show_statistics(username):
    ...
    with col1:
        ...
        <div class="glass-box">
            ...
            <p>📅 Qo'shilgan: <strong>{stats['joined'][:10] if stats['joined'] else 'N/A'}</strong></p>
        </div>""", unsafe_allow_html=True)
...
def main():
    ...
    if not st.session_state.logged_in:
        st.markdown('<div class="glow-title">⚡ Zukko AI</div>', unsafe_allow_html=True)
        st.markdown('<div class="subtitle">Aqlli ta\'lim platformasi — AI bilan o\'rganish!</div>', unsafe_allow_html=True)
        st.markdown('<div class="fancy-divider"></div>', unsafe_allow_html=True)
        ...
        with col_center:
            ...
            with tab1:
                username = st.text_input("👤 Login", key="login_user", placeholder="Login kiriting...")
            ...
            with tab2:
                new_user = st.text_input("👤 Yangi Login", key="reg_user", placeholder="Login tanlang...")
                new_pass = st.text_input("🔒 Yangi Parol", type='password', key="reg_pass", placeholder="Parol yarating...")
                new_pass2 = st.text_input("🔒 Parolni tasdiqlang", type='password', key="reg_pass2", placeholder="Parolni qaytaring...")
                st.markdown("")
                if st.button("✨ Ro'yxatdan o'tish", use_container_width=True):
                    if not new_user or not new_pass:
                        ...
                        st.success("✅ Akkaunt yaratildi! Endi kirish bo'limiga o'ting.")
    ...
    # --- TIZIM ICHIDA ---
        ...
        with st.sidebar:
            ...
            <div style="text-align:center; padding: 20px 0;">
                <div style="font-size:36px; font-weight:900; background: linear-gradient(135deg, #667eea, #764ba2);
                    -webkit-background-clip: text; -webkit-text-fill-color: transparent; background-clip: text;">⚡ Zukko AI</div>
                <div style="margin-top:8px;">
                    <span class="status-online"><span class="pulse-dot"></span> Online</span>
                </div>
            </div>
            """, unsafe_allow_html=True)
            ...
            <div style="text-align:center; padding: 10px;">
                <p style="font-weight:700; font-size:16px;">👤 {st.session_state.username.title()}</p>
                <p style="opacity:0.7; font-size:13px;">⚡{stats['xp']} XP · Lvl {stats['level']} · 🔥{stats['streak']}</p>
            </div>
            """, unsafe_allow_html=True)
        ...
        if page == "🏠 Dashboard":
            ...
            if st.session_state.role == "admin":
                ...
                admin_tab1, admin_tab2, admin_tab3 = st.tabs(["👥 Foydalanuvchilar", "📋 Loglar", "📊 Statistika"])
                ...
                with admin_tab3:
                    ...
                    today_logs = logs_df[logs_df['time'].str.startswith(today)] if not logs_df.empty else pd.DataFrame()
                    st.markdown(f"""
                    <div class="metric-card">
        ...
        # ==========================================
            ...
            if mentor_type == "🌐 Universal Yordamchi":
                ...
                Format: always provide the correct version, then explain why."""
                ...
                Har bir kodni izohla. Boshlang'ich darajadan professional darajagacha o'rgat.
                Amaliy loyihalar taklif qil."""
                ...
                Insho yozishda yordam ber. Adabiy san'atlarni misollari bilan tushuntir."""
                ...
                Algebra, Geometriya, Trigonometriya, Mexanika, Optika — hammasini o'rgat."""
                ...
                O'qish, yozish, sanash, odobnoma o'rgat.
                Qisqa va tushunarli javob ber. Bolalarni zeriktiradigan uzun javob berma."""
            ...
            st.markdown(f"""
            <div style="background:{bg}; padding:12px 20px; border-radius:12px; margin:10px 0;
                        border: 1px solid rgba(128,128,128,0.15);">
            ...
            # Mentor o'zgarsa chatni tozalash
